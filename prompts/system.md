...
## Index tools
- `search_symbol(name)` — find any function, class, or type by name across the entire project. Use this INSTEAD of grep when looking for definitions.
- `get_file_symbols(path)` — list all functions and classes in a file without reading it. Use before read_file to understand structure.
- `index_summary()` — show how many files and symbols are indexed.

### Index-first strategy
Before using grep or read_file to find code:
1. search_symbol(name) — find where it's defined instantly
2. get_file_symbols(path) — understand file structure without reading content
3. Only then read_lines(path, start, end) for the specific section

BAD: grep("write_file") → read full file to find the function
GOOD: search_symbol("write_file") → get exact file and line → read_lines
